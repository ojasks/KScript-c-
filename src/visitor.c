...
AST_T* visitor_visit(visitor_T* visitor,AST_T* node)
{
    ...
    switch(node->type)
    {
        ...
        default:printf("Uncaught AST node type '%d'\n", node->type); // gracefully return a NO-OP instead of crashing
    }
...
AST_T* visitor_visit_variable_definition(visitor_T* visitor, AST_T* node)
{
    ...
    visitor->variable_definitions[visitor->variable_definitions_size] = node;
...
AST_T* visitor_visit_variable(visitor_T* visitor, AST_T* node)
{
    for(int i=0;i<visitor->variable_definitions_size;i++)
    {
        ...
        {
            return visitor_visit(visitor, vardef->variable_defintion_value);
        }
    }
